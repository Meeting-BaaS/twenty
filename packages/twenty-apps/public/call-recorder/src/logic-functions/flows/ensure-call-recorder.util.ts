...
import { scheduleCallRecorderBot } from 'src/logic-functions/providers/schedule-call-recorder-bot.util';
import { updateCallRecording } from 'src/logic-functions/data/update-call-recording.util';

// The sole place a provider bot is created. Only the deterministic-create winner and the stale-state cron call it, so one writer per meeting POSTs exactly one bot.
export const ensureCallRecorder = async (
  ...
  if (isUndefined(workspaceId)) {
    console.error(
      `[call-recorder] cannot schedule provider bot for callRecording ${callRecording.id}: workspace id unavailable, the shared webhook could not be routed back`,
    );

    return false;
  }

  const scheduleResult = await scheduleCallRecorderBot({
    meetingUrl,
    joinAt,
  ...
  if (!scheduleResult.ok) {
    console.warn(
      `[call-recorder] failed to schedule provider bot for callRecording ${callRecording.id}: ${scheduleResult.errorMessage}`,
    );
